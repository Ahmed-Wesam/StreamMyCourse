...
- `./scripts/cfn-lint-templates.sh` (or `.\scripts\cfn-lint-templates.ps1`)
...
- [x] **Frontend** — [`VideoPlayer.tsx`](frontend/src/pages/lesson-player/VideoPlayer.tsx): passes `watermark={{ text, mode: 'random' }}` when the API includes `watermarkText`; playback profile gate is **API-only** (403 `watermark_profile_incomplete` → [`apiUserMessages.ts`](frontend/src/lib/apiUserMessages.ts)).
