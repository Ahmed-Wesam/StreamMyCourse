...
POST   /courses/{id}/modules/{mid}/quiz/start  // **QB-F:** Student start or resume module quiz; Cognito required. Same visibility gate as QB-D `moduleQuiz` (published course + lesson access + published bank + `served_count_n` set); failures → **404** `not_found` (not **403**). **Idempotent:** first call draws **N** published questions without replacement and persists binding; repeat calls return the same question **ids** in binding storage order. **200** + `{ "moduleQuizId", "moduleId", "servedCountN", "questions": [{ "id", "promptText", "optionsJson" }] }` — `optionsJson` is parsed JSON (choices array/object); **never** `correctOptionKey`, `questionBankId`, or bank `status`. **401** if unauthenticated. **409** `conflict` if published corpus < N at draw time, binding incomplete, or fewer than N questions load from RDS. Student SPA: **Start quiz** on course detail → [`ModuleQuizPage.tsx`](frontend/src/pages/ModuleQuizPage.tsx) via [`StudentModuleQuizAuth.tsx`](frontend/src/components/auth/StudentModuleQuizAuth.tsx). **QB-G** (deferred): per-attempt option shuffle (§8.4). **QB-H** (deferred): submit / scoring (§10–§11). Normative: [`plans/question-banks-requirements.md`](plans/question-banks-requirements.md) §7–§8.
...
| 4 | **RDS PostgreSQL (catalog)** | **Shipped and live in deployed dev/prod:** [`rds-stack.yaml`](infrastructure/templates/rds-stack.yaml), PostgreSQL adapters (`services/*/rds_repo.py`), migrator [`scripts/migrate-dynamodb-to-rds.py`](scripts/migrate-dynamodb-to-rds.py). **DynamoDB catalog fully removed** — api stack now requires `RdsStackName` and uses RDS exclusively. See [ADR-0008](plans/architecture/adr-0008-dynamodb-to-rds-migration.md) and [`tests/integration/README.md`](tests/integration/README.md). **Question banks:** migrations **006**–**008**; **QB-B** create bank + module quiz; **QB-C/QB-E** draft question + publish ([`mcq_validation.py`](infrastructure/lambda/catalog/services/question_banks/mcq_validation.py)); **QB-D** optional `moduleQuiz` on `GET /courses/{id}/modules` ([`visibility.py`](infrastructure/lambda/catalog/services/question_banks/visibility.py)); **QB-F** `POST .../quiz/start` + binding tables ([`binding_draw.py`](infrastructure/lambda/catalog/services/question_banks/binding_draw.py), [`StudentQuizStartDto`](infrastructure/lambda/catalog/services/question_banks/contracts.py)), student **Start** + [`ModuleQuizPage.tsx`](frontend/src/pages/ModuleQuizPage.tsx) via [`StudentModuleQuizAuth.tsx`](frontend/src/components/auth/StudentModuleQuizAuth.tsx) (prompts only — no Submit). API **CatalogApiDeploymentV21** in [`api-stack.yaml`](infrastructure/templates/api-stack.yaml). Unit: [`tests/unit/services/question_banks/`](tests/unit/services/question_banks/) (bindings repo, draw, start service/controller, visibility, publish). HTTPS: [`test_question_bank_start.py`](tests/integration/test_question_bank_start.py) + permissions/publish/visibility (dev deploy + schema **008**). Normative: [`plans/question-banks-requirements.md`](plans/question-banks-requirements.md) §5–§9. **Remaining:** **QB-G** shuffle / in-progress attempts; **QB-H/I** submit + scoring; HTTP for post-publish question add / draft update-delete (service methods exist). |
