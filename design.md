...
| 3 | **Auth** | **Shipped:** Cognito pool + required Google IdP + Google-only student/teacher app clients; API authorizer + `GET /users/me` + SPA Amplify Hosted UI OAuth + **`SignIn`** shell. **Ops:** GitHub Environment **`prod`** **`TEACHER_*` / `STUDENT_*` callback URLs**, **`GOOGLE_OAUTH_*`** secrets (required for full deploy), and API **`CorsAllowOrigin`** must match hosted origins. **Remaining:** tighten which routes are public vs Cognito-only (catalog still has open reads by design). |
