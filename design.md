...
```
GET    /courses/{id}/modules             // List modules (order, title, description); public for PUBLISHED; DRAFT 404 unless owner/admin (same rule as lesson list). **QB-D:** optional per-module `moduleQuiz` when visibility passes — `{ "available": true, "servedCountN": <n> }` (draw size only; no question text). Omitted when course is DRAFT, viewer lacks lesson access (not enrolled and not publisher/admin), bank is not PUBLISHED, or `served_count_n` is unset.
POST   /courses/{id}/modules             // Create module (body: title, optional description); Cognito teacher/admin when enforced
POST   /courses/{id}/question-banks      // Create DRAFT question bank for course; Cognito teacher/admin; **course publisher** (or admin) only — same `courses.created_by` vs `sub` rule as other course mutations. **201** + `{ "questionBankId": "<uuid>" }`
POST   /courses/{id}/question-banks/{bid}/questions  // Add DRAFT MCQ row (`promptText`, `optionsJson` array/object with ≥1 choice `key`, optional `correctOptionKey` must match a choice key when set); publisher-only; **201** + `{ "questionId": "<uuid>" }`
...
POST   /courses/{id}/modules/{mid}/quiz  // Create module quiz row (optional body `questionBankId`); same publisher auth as above. **201** + `{ "quizId": "<uuid>" }`
POST   /courses/{id}/modules/{mid}/quiz/start  // **QB-F + QB-G:** Student start or resume module quiz; Cognito required. Same visibility gate as QB-D `moduleQuiz` (published course + lesson access + published bank + `served_count_n` set); failures → **404** `not_found` (not **403**). **Binding (QB-F):** first call draws **N** published questions without replacement and persists binding; repeat calls keep the same bound question **identities** (draw order in storage). **Attempt + shuffle (QB-G, migration **009**):** resolves or creates one **`in_progress`** attempt per binding (`module_quiz_attempts`); first open attempt gets `attemptNumber` **1** (or next after prior attempts); persists `shuffled_question_order` + per-question `shuffled_choice_orders`; re-`start` while that attempt is open returns the **same** `attemptId`, `questionIds`, and shuffled `questions` (§8.3–§8.4). **200** + `{ "moduleQuizId", "moduleId", "servedCountN", "attemptId", "attemptNumber", "questionIds", "questions": [{ "id", "promptText", "optionsJson" }] }` — `questionIds` is the attempt’s shuffled question order; `optionsJson` is parsed JSON with choices reordered per attempt (never `correctOptionKey`, `questionBankId`, or bank `status`). **401** if unauthenticated. **409** `conflict` if published corpus < N at draw time, binding incomplete, shuffle validation fails, or fewer than N questions load from RDS. Student SPA: **Start quiz** on course detail → [`ModuleQuizPage.tsx`](frontend/src/pages/ModuleQuizPage.tsx) via [`StudentModuleQuizAuth.tsx`](frontend/src/components/auth/StudentModuleQuizAuth.tsx) (no Submit). **QB-H** (deferred): submit / scoring (§10–§11). Normative: [`plans/question-banks-requirements.md`](plans/question-banks-requirements.md) §7–§8.
...
| 4 | **RDS PostgreSQL (catalog)** | **Shipped and live in deployed dev/prod:** [`rds-stack.yaml`](infrastructure/templates/rds-stack.yaml), PostgreSQL adapters (`services/*/rds_repo.py`), migrator [`scripts/migrate-dynamodb-to-rds.py`](scripts/migrate-dynamodb-to-rds.py). **DynamoDB catalog fully removed** — api stack now requires `RdsStackName` and uses RDS exclusively. See [ADR-0008](plans/architecture/adr-0008-dynamodb-to-rds-migration.md) and [`tests/integration/README.md`](tests/integration/README.md). **Question banks:** migrations **006**–**009**; **QB-B** create bank + module quiz; **QB-C/QB-E** draft question + publish ([`mcq_validation.py`](infrastructure/lambda/catalog/services/question_banks/mcq_validation.py)); **QB-D** optional `moduleQuiz` on `GET /courses/{id}/modules` ([`visibility.py`](infrastructure/lambda/catalog/services/question_banks/visibility.py)); **QB-F** binding draw + **QB-G** in-progress attempts + presentation shuffle on `POST .../quiz/start` ([`binding_draw.py`](infrastructure/lambda/catalog/services/question_banks/binding_draw.py), [`presentation_shuffle.py`](infrastructure/lambda/catalog/services/question_banks/presentation_shuffle.py), [`009_module_quiz_attempts.sql`](infrastructure/database/migrations/009_module_quiz_attempts.sql), [`StudentQuizStartDto`](infrastructure/lambda/catalog/services/question_banks/contracts.py)), student **Start** + [`ModuleQuizPage.tsx`](frontend/src/pages/ModuleQuizPage.tsx) via [`StudentModuleQuizAuth.tsx`](frontend/src/components/auth/StudentModuleQuizAuth.tsx) (prompts only — no Submit). API **CatalogApiDeploymentV21** in [`api-stack.yaml`](infrastructure/templates/api-stack.yaml). Unit: [`tests/unit/services/question_banks/`](tests/unit/services/question_banks/) (bindings, attempts repo, presentation shuffle, draw, start service/controller, visibility, publish). HTTPS: [`test_question_bank_start.py`](tests/integration/test_question_bank_start.py) + permissions/publish/visibility (dev deploy + schema **008+009**). Normative: [`plans/question-banks-requirements.md`](plans/question-banks-requirements.md) §5–§9. **Remaining:** **QB-H/I** submit + scoring; HTTP for post-publish question add / draft update-delete (service methods exist). |
