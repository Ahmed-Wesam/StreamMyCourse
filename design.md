...
| 6 | **Billing / subscriptions (WS2+)** | In progress — WS1–**WS7** shipped through student **manage/reactivate** ([`plans/billing-subscription-mega-plan.md`](plans/billing-subscription-mega-plan.md), [WS7 child plan](plans/billing-workstream-7-subscription-manage-reactivate.md)): **subscription-only** access; **`POST …/enroll`** → **403** `subscription_required`; **`POST /billing/checkout-session`** (mock HPP) + paywall (**50 JOD/month**); checkout gates per [`subscribe-contract-v1.md`](plans/billing/subscribe-contract-v1.md); **`GET /billing/subscription`**, **`POST /billing/cancel-subscription`**, **`POST /billing/reactivate-subscription`** per [`manage-contract-v1.md`](plans/billing/manage-contract-v1.md); student **`/account`** (profile + manage subscription). PayTabs **`cancel_agreement`** deferred to period end (**WS8** scheduler; **WS9** live flip). **Remaining:** [WS8](plans/billing-workstream-8-billing-ops-pre-go-live.md) ops (no account), [WS9](plans/billing-workstream-9-paytabs-live-go-live.md) live PayTabs (account required), [WS10](plans/billing-workstream-10-adr-contract-documentation.md) API docs. |
