...
| 6 | **Billing / subscriptions (WS2+)** | In progress — WS1 schema + WS2 payments + **WS3 IPN/fulfillment** + **WS5 catalog access** + **WS6 student checkout** ([`plans/billing-subscription-mega-plan.md`](plans/billing-subscription-mega-plan.md)): **subscription-only** lesson/playback/progress/quiz access enforced; **`POST …/enroll`** returns **403** `subscription_required`; **`POST /billing/checkout-session`** (mock HPP) + paywall UI (**50 JOD/month**, migration **012**); checkout gates per [`subscribe-contract-v1.md`](plans/billing/subscribe-contract-v1.md) (`billing_unconfigured` → `reactivation_required` → `already_subscribed`). **Remaining:** WS7 manage/reactivate, WS8 live PayTabs, WS9 API docs. |
