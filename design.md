...
- **Video provider edge (Kinescope, no NAT):** [`infrastructure/templates/video-provider-edge-stack.yaml`](infrastructure/templates/video-provider-edge-stack.yaml) deploys **`StreamMyCourse-VideoProviderEdge-{env}`** — a **no-VPC** Lambda ([`infrastructure/lambda/video_provider_edge/`](infrastructure/lambda/video_provider_edge/)) for outbound Kinescope HTTP. [`scripts/deploy-video-provider-edge.sh`](scripts/deploy-video-provider-edge.sh) packages and deploys the edge stack; [`scripts/deploy-backend.sh`](scripts/deploy-backend.sh) runs it only when `VIDEO_PROVIDER=kinescope` (dev default; prod defaults to `s3` until cutover). **Route split:** with `UseKinescopeVideoEdge`, API Gateway sends **`POST /upload-url`**, **`PUT …/video-ready`**, and **`POST /webhooks/kinescope`** to the edge Lambda; playback, DRM auth webhook, and all other routes stay on the catalog Lambda in the RDS VPC. Edge invokes catalog synchronously for domain writes via `internal` events in [`internal_video.py`](infrastructure/lambda/catalog/services/course_management/internal_video.py). **Deploy phases:** **A** — initial api-stack deploy (catalog ARN exists); **C** — edge stack with `CatalogLambdaArn`; **D** — api-stack re-deploy with `VideoProviderEdgeLambdaArn` to wire routes. CI deploy ([`.github/workflows/deploy-backend.yml`](.github/workflows/deploy-backend.yml)) follows the same sequence for dev when Kinescope is enabled.
...
| 6 | **Billing / subscriptions (WS2+)** | In progress — WS1–**WS8** shipped in repo ([`plans/billing-subscription-mega-plan.md`](plans/billing-subscription-mega-plan.md)): **subscription-only** access; **`POST /billing/checkout-session`** (mock HPP); **`GET /billing/subscription`**, **`POST /billing/cancel-subscription`** (RDS cancel-at-period-end + immediate **`cancel_agreement`** on billing edge); student **`/account`**; checkout **`already_subscribed`** for any in-period granting access (including canceled-in-period). **No reactivate.** Ops: [`billing-ops-runbook.md`](infrastructure/docs/billing-ops-runbook.md). Pre-rollout: **`PAYTABS_USE_MOCK=true`**. **Remaining:** [WS9](plans/billing-workstream-9-paytabs-live-go-live.md) live PayTabs, [WS10](plans/billing-workstream-10-adr-contract-documentation.md). |
| 7 | **Legal / merchant disclosure** | **Shipped (frontend):** bilingual **Privacy** + **Terms** on student SPA ([`legalConfig.ts`](frontend/src/lib/legalConfig.ts), [`docs/legal/`](docs/legal/)); teacher footer + PayTabs setup use absolute student URLs via [`legalUrls.ts`](frontend/src/lib/legalUrls.ts). **Remaining:** counsel review of EN/AR copy before PayTabs go-live. |
