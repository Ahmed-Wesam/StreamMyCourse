...
GET    /courses/{id}                     // Full details for catalog; may include thumbnailUrl + enrolled (bool); **no** API Gateway Cognito authorizer on this method — PUBLISHED is public, DRAFT is 404 unless caller is owner/admin (Lambda + JWT claims when present)
...
### Lessons
```
GET    /courses/{id}/lessons             // Lesson rows for catalog UI (order, title, videoStatus, optional thumbnailUrl presigned GET); **no** videoKey in JSON; **no** API Gateway Cognito authorizer — PUBLISHED is public, DRAFT 404 unless owner/admin. Playback remains on GET /playback/...
...
- **Optional auth:** When `CognitoUserPoolArn` is set on the API stack, API Gateway uses a **Cognito user pools authorizer** on protected routes (mutations, `/courses/mine`, **`GET /playback/...`**, etc.) and Lambda enforces role/ownership checks (`COGNITO_AUTH_ENABLED=true`). **`GET /courses/{id}`** and **`GET /courses/{id}/lessons`** intentionally have **`AuthorizationType: NONE`** so anonymous browsers can load catalog pages; the Lambda still hides **DRAFT** from non-managers and never returns **`videoKey`**. Thumbnail presigned URLs in those responses are effectively **public** for **PUBLISHED** courses — do not put PII or paid-only content in cover/lesson thumbnail images.
...
- Lambda code packaged as a zip and uploaded to an artifacts S3 bucket; stack references `LambdaCodeS3Bucket` / `LambdaCodeS3Key`. **CI and [`deploy.ps1`](infrastructure/deploy.ps1)** use a **git-SHA-based key** (`catalog-{env}-{sha}.zip`) so each deploy changes the parameter and the stack updates Lambda (fixed keys caused empty changesets). The same key is referenced from **`CatalogApiDeploymentV9.Description`** in [`infrastructure/templates/api-stack.yaml`](infrastructure/templates/api-stack.yaml) so **API Gateway** also receives a **new deployment** when the artifact changes (avoids a stage serving an older snapshot while methods in the console already show Cognito). The deployment resource logical id is occasionally bumped so template-only route changes still replace the stage snapshot.
