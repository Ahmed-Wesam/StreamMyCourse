...
| 4 | **RDS PostgreSQL (catalog)** | **Shipped and live in deployed dev/prod:** [`rds-stack.yaml`](infrastructure/templates/rds-stack.yaml), PostgreSQL adapters (`services/*/rds_repo.py`), migrator [`scripts/migrate-dynamodb-to-rds.py`](scripts/migrate-dynamodb-to-rds.py). **DynamoDB catalog fully removed** — api stack now requires `RdsStackName` and uses RDS exclusively. See [ADR-0008](plans/architecture/adr-0008-dynamodb-to-rds-migration.md) and [`tests/integration/README.md`](tests/integration/README.md). **Question banks (repo + pipeline):** migrations **006**–**010** ([`deploy-backend.yml`](.github/workflows/deploy-backend.yml) apply-schema lists include [`010_module_quiz_attempt_submissions.sql`](infrastructure/database/migrations/010_module_quiz_attempt_submissions.sql)); **QB-B** create bank + module quiz; **QB-C/QB-E** draft question + publish ([`mcq_validation.py`](infrastructure/lambda/catalog/services/question_banks/mcq_validation.py)); **QB-D** optional `moduleQuiz` on `GET /courses/{id}/modules` ([`visibility.py`](infrastructure/lambda/catalog/services/question_banks/visibility.py)); **QB-F** binding draw + **QB-G** attempts + shuffle + **QB-H/I** submit, equal-weight grading ([`grading.py`](infrastructure/lambda/catalog/services/question_banks/grading.py)), discriminated **`POST .../quiz/start`** (`phase` `in_progress` \| `latest_results`, optional `retake`, `latestSubmission`), and **`POST .../quiz/submit`** ([`contracts.py`](infrastructure/lambda/catalog/services/question_banks/contracts.py), [`controller.py`](infrastructure/lambda/catalog/services/question_banks/controller.py)); student quiz UI [`ModuleQuizPage.tsx`](frontend/src/pages/ModuleQuizPage.tsx) + [`api.ts`](frontend/src/lib/api.ts). API **CatalogApiDeploymentV22** in [`api-stack.yaml`](infrastructure/templates/api-stack.yaml). Unit: [`tests/unit/services/question_banks/`](tests/unit/services/question_banks/) (incl. submit, grading, submissions repo). Integration: [`test_question_bank_start.py`](tests/integration/test_question_bank_start.py), [`test_question_bank_submit.py`](tests/integration/test_question_bank_submit.py) (authored for HTTPS post-deploy), permissions/publish/visibility. Normative: [`plans/question-banks-requirements.md`](plans/question-banks-requirements.md) §5–§11. **Remaining (repo backlog):** **QB-J** cross-stack audit ([`plans/question-banks-mega-plan.md`](plans/question-banks-mega-plan.md)); HTTP for post-publish question add / draft update-delete (service methods exist). |
