...
def test_handle_users_me_returns_401_when_missing_sub(monkeypatch, make_lambda_event) -> None:
    ...
    auth_svc = MagicMock()
    auth_svc.get_or_create_profile.side_effect = Unauthorized("Authentication required")

    resp = handle_users_me(evt, origin="*", auth_svc=auth_svc)
...
def test_handle_users_me_prefers_custom_role_over_role(monkeypatch, make_lambda_event) -> None:
    ...
    evt["requestContext"]["authorizer"] = {
        "claims": {"sub": "u1", "email": "a@b.com", "custom:role": "teacher", "role": "student"}
    }
    auth_svc = MagicMock()
...
def test_handle_users_me_maps_http_error_to_json(monkeypatch, make_lambda_event) -> None:
    evt = make_lambda_event(method="GET", path="/users/me")
    evt["requestContext"]["authorizer"] = {
        "claims": {"sub": "u1", "email": "a@b.com", "custom:role": "student"}
    }
    auth_svc = MagicMock()
    auth_svc.get_or_create_profile.side_effect = BadRequest("bad", code="X")

    resp = handle_users_me(evt, origin="https://app.example", auth_svc=auth_svc)
