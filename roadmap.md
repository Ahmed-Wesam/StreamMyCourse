...
> **Status:** **Post-MVP evolution** (MVP baseline shipped; Phase 2+ ahead) · **Last updated:** 2026-05-15
...
- **Frontend:** React 19 (Vite + TS + Tailwind), student **Home** (`/`), published **catalog** at **`/catalog`**, signed-in **My Course** hub at **`/my-course`**, course detail (including a **pricing** section component for Figma parity—still no paid checkout in MVP), lesson player (including **module quiz** entry in the sidebar and **Next** to `/courses/:courseId/modules/:moduleId/quiz` after the last lesson in a module when `moduleQuiz` is available), plus instructor dashboard and course management. **Hosted** as **two SPAs** (student + teacher) on S3 + CloudFront + Route 53 via **`StreamMyCourse-EdgeHosting-{env}`** in **`us-east-1`** (unified stack). On **`main`**, **[`deploy-backend.yml`](.github/workflows/deploy-backend.yml)** drives dev/prod SPA asset deploys after edge (with **[`deploy-web-reusable.yml`](.github/workflows/deploy-web-reusable.yml)** / teacher reusable). Student UI gaps vs Figma/backend are listed in **[`reports/figma-student-ui-gap-report.md`](reports/figma-student-ui-gap-report.md)**.
...
| Order | Track | Outcome |
|-------|--------|---------|
| 1 | **Video CDN** | **Shipped (baseline):** CloudFront + OAC on the private video bucket in [`video-stack.yaml`](infrastructure/templates/video-stack.yaml); playback URLs remain presigned S3 until product switches. **Phase 2:** signed CloudFront URLs, WAF, cost tuning. |
