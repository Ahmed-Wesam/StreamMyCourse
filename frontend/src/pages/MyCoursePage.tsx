...
export default function MyCoursePage() {
  ...
  const continueTo =
    state.status === 'ready' && featured && resume
      ? resume.startTimeSec > 0
        ? {
            pathname: `/courses/${featured.id}/lessons/${resume.lesson.id}`,
            search: `?t=${resume.startTimeSec}`,
          }
        : `/courses/${featured.id}/lessons/${resume.lesson.id}`
      : null
          ...
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center lg:gap-x-14">
            <div className="min-w-0">
              ...
              <div className="mt-9 flex flex-wrap gap-4">
                {state.status === 'loading' ? null : continueTo ? (
                  <Link
                    to={continueTo}
                    ...
                    <div>
                      ...
                      <div className="mt-2 h-2 rounded-full bg-white/10">
                        <div
                          ...
                          aria-valuemin={0}
                          aria-valuemax={100}
                          aria-valuenow={Math.min(100, Math.max(0, percentComplete))}
