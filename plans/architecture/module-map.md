...
| `service.py` | Domain/application | `QuestionBankService` — authorizer before every repo write; publish §9.3 / N §5; draft + published-question rules; **QB-L:** `list_module_quizzes_for_course`; **`create_module_quiz`** requires existing `questionBankId` for the same course; **QB-F + QB-G + QB-I:** `start_module_quiz` (QB-D gate + idempotent binding + resolve/create `in_progress` or return `latest_results` + `latestSubmission`; optional `retake` + fresh shuffle); **QB-H:** `submit_module_quiz` |
| `controller.py` | HTTP adapter | Publisher: `POST .../question-banks`, `GET .../module-quizzes`, `.../questions`, `.../publish`, `POST .../modules/{mid}/quiz` (body requires `questionBankId`). Student: **`POST .../modules/{mid}/quiz/submit`**, **`POST .../quiz/start`** ( **`quiz/submit` before `quiz/start` before bare `quiz`** ). Cognito claims → service |
